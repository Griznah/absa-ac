...
// TestStaticFileServer_Normal tests that file server returns index.html for root path
func TestStaticFileServer_Normal(t *testing.T) {
	tmpDir := t.TempDir()
	...
	// Change to temp directory (FileServer uses "./static" relative path)
	origWd, _ := os.Getwd()
	os.Chdir(tmpDir)
	defer os.Chdir(origWd)

	// Create FileServer handler (mimics main.go setup)
	...
	body := make([]byte, len(indexContent))
	...
	if string(body) != indexContent {
		t.Errorf("Response body mismatch\nExpected: %s\nGot: %s", indexContent, string(body))
	}
}

// TestStaticFileServer_NotFound tests that request for non-existent file returns 404
...
// TestStaticFileServer_DirectoryTraversal tests that directory traversal attempts return 403
func TestStaticFileServer_DirectoryTraversal(t *testing.T) {
	tmpDir := t.TempDir()

	// Create static directory with a test file
	staticDir := filepath.Join(tmpDir, "static")
	os.MkdirAll(staticDir, 0755)
	...
	if resp.StatusCode != http.StatusForbidden {
		t.Errorf("Expected status 403 for directory traversal, got %d", resp.StatusCode)
	}
}

// TestStaticFileServer_MIMETypes tests that static files have correct MIME types
func TestStaticFileServer_MIMETypes(t *testing.T) {
	...
	os.WriteFile(filepath.Join(staticDir, "test.html"), []byte("<html></html>"), 0644)
	os.WriteFile(filepath.Join(staticDir, "test.js"), []byte("console.log('test');"), 0644)
	os.WriteFile(filepath.Join(staticDir, "test.css"), []byte("body {}"), 0644)
	os.WriteFile(filepath.Join(staticDir, "test.mjs"), []byte("export default {};"), 0644)
	...
	for _, tc := range testCases {
		t.Run(tc.filePath, func(t *testing.T) {
			resp, err := http.Get(server.URL + tc.filePath)
			if err != nil {
				t.Fatalf("GET %s failed: %v", tc.filePath, err)
			}
			defer resp.Body.Close()
